...
`mdpresent`는 **NO LLM runtime** 엔진입니다. 파싱, 분할, 레이아웃, 검증, 렌더링은 외부 API 호출 없이 deterministic rule-based 방식으로 동작합니다. 주 출력물은 editable `PPTX`이며, HTML은 가벼운 미리보기와 갤러리 shell로만 사용됩니다.
...
[테마 미리보기](https://ch040602.github.io/MdPr/theme-preview/)는 Markdown에서 먼저 PPTX를 생성한 뒤, 해당 PPTX 슬라이드를 PNG로 추출하여 보여줍니다. 각 테마별 PPTX 파일도 함께 내려받아 확인할 수 있습니다.
...
5. `packages/render-pptx`를 editable object 중심의 주 렌더러로 유지한다.
6. `packages/render-html`은 브라우저 미리보기와 갤러리 shell로 유지한다.
7. `packages/render-pdf`는 문서 export 경로로 유지한다.
